...
const LoginScreen = ({navigation}) => {
  ...
  // email has to be atleast 4 characters long
  ...
  const loginHandle = async (email, password) => {
    ...
    try {
      ...
      // User found
        ...
        var storeUsername = async (value) => {
           try {
             await AsyncStorage.setItem('@username_Key', value)
             Alert.alert('We successfully stored ' + value)
           } catch (e) {
             Alert.alert('error on storage')
           }
         }

         var storeEmail =  (value) => {
            try {
